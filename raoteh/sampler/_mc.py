...
def construct_node_to_conditional_pmap(T, root, node_to_allowed_states):
    """
    For each node, construct the map from state to subtree likelihood.
            ...
            for node_state in valid_node_states:
                ...
                if not structural_failure:
                    ...
                    for n in successors[node]:
                        ...
                        cond_weight = {}
                        for s in valid_states:
                            cond_weight[s] = P[node_state][s]['weight']
                        total = np.sum(cond_weight.values())
                        pairs = cond_weight.items()
                        cond_distn = dict((s, w / total) for s, w in pairs)

                        # Apply the probability for the current branch.
                        nprob = 0.0
                        for s in valid_states:
                            #a = P[node_state][s]['weight']
                            a = cond_distn[s]
